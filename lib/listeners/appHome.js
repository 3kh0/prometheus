import { isGlobalAdmin, listGlobalAdmins, listAllAppointedManagers, listAllChannelBans, removeChannelBan, addGlobalAdmin, removeGlobalAdmin, addAppointedManager, removeAppointedManager } from '../db.js';
  ...
  if (!bans.length) {
    ...
    for (const [ch, entries] of Object.entries(byChannel)) {
      ...
      for (const b of entries) {
        const expiry = b.expires
          ? `expires <!date^${b.expires}^{date_short_pretty} at {time}|${new Date(b.expires * 1000).toUTCString()}>`
          : 'permanent';
        const reason = b.reason ? ` — _${b.reason}_` : '';
        blocks.push({
          type: 'section',
          text: { type: 'mrkdwn', text: `:bonk: <@${b.user_id}> timed out by <@${b.banned_by}> (${expiry})${reason}` },
          accessory: {
            type: 'button',
            text: { type: 'plain_text', text: 'Remove Timeout' },
            style: 'danger',
            action_id: `home_unban_${b.user_id}_${b.channel_id}`,
            confirm: {
              title: { type: 'plain_text', text: 'Remove Timeout?' },
              text: { type: 'plain_text', text: `Remove the timeout on this user in this channel?` },
...
export const actions = [
  {
    ...
    async execute({ action, body, client }) {
      ...
      logAdmin(client, { action: `removed <@${target}> as global admin`, adminUser: body.user.id, channel: 'global', detail: 'via App Home' });
  ...
  },
  {
    ...
    async execute({ action, body, client }) {
      ...
      logAdmin(client, { action: `removed <@${uid}> from channel roles`, adminUser: body.user.id, channel: ch, detail: 'via App Home' });
      await refreshHome(client, body.user.id);
    },
  },
  {
    pattern: /^home_unban_(.+?)_(.+)$/,
    async execute({ action, body, client }) {
      if (!isGlobalAdmin(body.user.id)) return;
      const [, uid, ch] = action.action_id.match(/^home_unban_(.+?)_(.+)$/);
...
export const views = [
  {
    ...
    async handleView({ view, body, client }) {
      ...
      logAdmin(client, { action: `added <@${target}> as global admin`, adminUser: invoker, channel: 'global', detail: 'via App Home' });
  ...
  },
  {
    ...
    async handleView({ view, body, client }) {
      ...
      logAdmin(client, { action: `added <@${uid}> as channel ${role}`, adminUser: invoker, channel: ch, detail: 'via App Home' });
      await refreshHome(client, invoker);
    },
  },
];
