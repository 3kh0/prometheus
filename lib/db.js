...
const banId = (userId, channelId) => `${userId}_${channelId}`;
...
const statements = {
  ...
  getChannelBan: db.query('SELECT ban_id, banned_by, reason, expires FROM channel_bans WHERE ban_id = $banId'),
  setChannelBan: db.query('INSERT OR REPLACE INTO channel_bans (ban_id, banned_by, reason, expires) VALUES ($banId, $bannedBy, $reason, $expires)'),
  removeChannelBan: db.query('DELETE FROM channel_bans WHERE ban_id = $banId'),
};
...
export function removeChannelBan(userId, channelId) {
  statements.removeChannelBan.run({ $banId: banId(userId, channelId) });
}
