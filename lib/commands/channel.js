...
export default {
  ...
  async execute({ command: cmd, args, respond, context, logger }) {
    ...
    if (!await canBan(context.userClient, cmd.user_id, cmd.channel_id)) {
      console.log(`[channel] ${cmd.user_id} denied in ${cmd.channel_id}`);
      return err(':loll: You do not have permission to use this command.');
    }

    if (!msg) return err(':red-x: Usage: `/pro channel <message>`');

    try {
      const u = (await context.userClient.users.info({ user: cmd.user_id })).user;
      const text = `@channel ${msg}`;
      const opts = {
      ...
      try {
        await post();
        console.log(`[channel] ${cmd.user_id} sent @channel in ${cmd.channel_id}`);
      } catch (e) {
        if (e.data?.error !== 'not_in_channel') throw e;
        console.log(`[channel] not in channel ${cmd.channel_id}, joining...`);
        try {
