...
export default {
  ...
  async execute({ command: cmd, args, respond, client, context }) {
    ...
    if (existing && (!existing.expires || existing.expires > Math.floor(Date.now() / 1000)))
      return err(`:red-x: <@${userId}> is already banned from <#${channelId}>.`);
    ...
    const banMsg = `You have been banned from <#${channelId}>${expires ? ` until <!date^${expires}^{date_short_pretty} at {time}|${dateFallback}>` : ''}${reason ? ` for the following reason: ${reason}` : '.'}`;
    ...
    await respond({
      ...
      text: `:bonk: Banned <@${userId}> from <#${channelId}>${expires ? ` until <!date^${expires}^{date_short_pretty} at {time}|${dateFallback}>` : ''}.`
    });
  }
};
